...
function DeleteConfirm({ test, onConfirm, onCancel }: { test: WritingTest; onConfirm: () => void; onCancel: () => void }) {
  return (
...
function WritingTestsPage() {
  ...
  const [deleting, setDeleting] = useState<WritingTest | null>(null);
  const [message, setMessage] = useState("");

  const addPath = `/admin/tests/${examType}/writing/add`;
  ...
  useEffect(() => {
    ...
    getAdminTests<WritingTest>(token, { kind: "writing", examType: exam })
      .then((result) => setTests(result.tests))
      .catch((error) => setMessage(error instanceof Error ? error.message : "Writing testlar yuklanmadi"));
  }, [exam]);

  async function handleDelete(test: WritingTest) {
    const token = getAdminToken();
    if (!token) {
      setMessage("Admin token topilmadi. Qayta login qiling.");
      return;
    ...
    await deleteAdminTest(token, "writing", test.id);
    ...
    <>
      <section className="admin-table-section">
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
          <AdminSectionTitle title={`${cfg.label} — Writing Tests`} description="List of all writing tasks." meta={`${tests.length} tests`} />
          ...
          <table className="admin-table">
            ...
            <tbody>
              {tests.map((test, idx) => {
                ...
                const isTask1  = test.task.type === "task1";
                return (
                  <tr key={test.id}>
                    ...
                    <td>
                      <span style={{ padding: "3px 10px", borderRadius: "999px", background: isTask1 ? "#dbeafe" : "#ede9fe", color: isTask1 ? "#1d4ed8" : "#7c3aed", fontSize: "var(--adm-fs-tag)", fontWeight: 900, whiteSpace: "nowrap" }}>
                        {isTask1 ? "Task 1 — Graph / Chart" : "Task 2 — Essay"}
                      </span>
                    </td>
                    <td>
                      <span style={{ padding: "3px 10px", borderRadius: "999px", background: lvlCfg.bg, color: lvlCfg.color, fontSize: "var(--adm-fs-tag)", fontWeight: 900 }}>
                        {test.level}
                      </span>
                    </td>
                    <td style={{ color: "#334155", fontSize: "var(--adm-fs-sm)", fontWeight: 700 }}>{test.task.wordLimit}</td>
                ...
                <tr>
                  <td colSpan={7}>
                    <div className="admin-dashboard-empty">
                      <strong>No writing tests yet</strong>
                      <span>
                        Click{" "}
                        <button type="button" onClick={() => navigate(addPath)} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontWeight: 900, fontSize: "inherit" }}>
                          "Add Test"
                        </button>{" "}
                        to create your first writing test.
                      </span>
                    </div>
                  </td>
                </tr>
      ...
      {deleting && (
        <DeleteConfirm test={deleting} onConfirm={() => handleDelete(deleting)} onCancel={() => setDeleting(null)} />
      )}
    </>
  );
}

export default WritingTestsPage;
