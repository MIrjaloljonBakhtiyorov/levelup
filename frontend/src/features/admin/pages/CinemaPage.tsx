...
import type { AdminCinema, CinemaLevel } from "../types/adminTypes";
import { createId, formatDate } from "../utils/adminFormatters";
import AdminSectionTitle from "../components/AdminSectionTitle";

const LEVELS: CinemaLevel[] = ["A1", "A2", "B1", "B2", "C1", "C2"];

const LEVEL_COLORS: Record<CinemaLevel, { bg: string; color: string }> = {
    ...
    return (
      <div style={{ position: "relative", paddingBottom: "56.25%", borderRadius: "10px", overflow: "hidden", background: "#000" }}>
        <iframe style={{ position: "absolute", inset: 0, width: "100%", height: "100%", border: "none" }}
          src={`https://www.youtube.com/embed/${ytId}`} title="Cinema video"
...
// ─── Modal ────────────────────────────────────────────────────────
function CinemaModal({ initial, onSave, onClose }: { initial?: AdminCinema; onSave: (c: AdminCinema) => void; onClose: () => void }) {
  ...
  const [level,   setLevel]   = useState<CinemaLevel>(initial?.level ?? "B1");
  ...
  function validate() {
    ...
    if (!title.trim()) e.title = "Enter cinema title";
    ...
    <div className="admin-profile-modal" role="dialog" aria-modal="true">
      <article className="admin-profile-modal__card" style={{ maxWidth: "520px", width: "100%" }}>
        <div className="admin-profile-modal__header">
          <strong style={{ fontSize: "var(--adm-fs-sub)" }}>{initial ? "Edit Cinema" : "Add Cinema"}</strong>
        ...
        <div style={{ display: "grid", gap: "18px", padding: "20px 0 4px" }}>
          ...
          {/* Title */}
          <div>
            <label style={lb}>Cinema title *</label>
            <input style={{ ...inp, borderColor: errors.title ? "#cf6d6d" : undefined }}
              value={title} placeholder="e.g. The King's Speech (2010)"
...
// ─── Cinema Card ──────────────────────────────────────────────────
function CinemaCard({ item, onEdit, onDelete }: { item: AdminCinema; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <article style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "14px", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
      <div style={{ padding: "12px 12px 0" }}>
        <VideoPreview url={item.videoUrl} />
      </div>
      <div style={{ padding: "14px 16px 16px", display: "flex", flexDirection: "column", gap: "8px", flex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "#64748b", fontSize: "var(--adm-fs-tag)" }}>
          {item.videoType === "link" ? (
            <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg><span>Link</span></>
          ) : (
            <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.videoFileName}</span></>
          )}
        </div>
...
function DeleteConfirm({ item, onConfirm, onCancel }: { item: AdminCinema; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="admin-profile-modal" role="dialog" aria-modal="true">
      <article className="admin-profile-modal__card" style={{ maxWidth: "420px" }}>
        <div className="admin-profile-modal__header">
          <strong style={{ fontSize: "var(--adm-fs-sub)", color: "#be123c" }}>Delete Cinema</strong>
...
// ─── Main Page ────────────────────────────────────────────────────
function CinemaPage() {
  const [items,       setItems]       = useState<AdminCinema[]>([]);
  ...
  const [levelFilter, setLevelFilter] = useState<CinemaLevel | "">("");
  ...
  useEffect(() => {
    ...
    getAdminContent<AdminCinema>(token, "cinema")
      .then((result) => setItems(result.items))
      .catch((error) => setMessage(error instanceof Error ? error.message : "Cinema contentni yuklab bo'lmadi"));
  }, []);
  ...
  async function handleSave(cinema: AdminCinema) {
    const token = getAdminToken();
    if (!token) {
      setMessage("Admin token topilmadi. Qayta login qiling.");
      return;
    ...
    try {
      const { id: _id, createdAt: _createdAt, ...payload } = cinema;
      const result = editing
        ? await updateAdminContent<AdminCinema>(token, "cinema", cinema.id, payload)
        : await createAdminContent<AdminCinema>(token, "cinema", payload);
      ...
      setMessage(error instanceof Error ? error.message : "Cinema content saqlanmadi");
    }
  }

  async function handleDelete(cinema: AdminCinema) {
    const token = getAdminToken();
    if (!token) {
      setMessage("Admin token topilmadi. Qayta login qiling.");
      return;
    ...
    try {
      await deleteAdminContent(token, "cinema", cinema.id);
      setItems((current) => current.filter((item) => item.id !== cinema.id));
      setDeleting(null);
      setMessage("");
      window.dispatchEvent(new Event("admin-data-changed"));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Cinema content o'chirilmadi");
    }
  }
    ...
    <>
      <section className="admin-table-section">
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
          <AdminSectionTitle title="Cinema" description="Manage all cinema videos available on the platform." meta={`${items.length} videos`} />
        ...
        {filtered.length > 0 ? (
          <div style={{ padding: "0 20px 24px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "20px" }}>
            {filtered.map((item) => (
              <CinemaCard key={item.id} item={item} onEdit={() => setEditing(item)} onDelete={() => setDeleting(item)} />
            ))}
          </div>
        ) : (
          <div className="admin-dashboard-empty" style={{ margin: "0 20px 24px" }}>
            <strong>{search || levelFilter ? "No videos match your filter" : "No cinema videos yet"}</strong>
            <span>
              {!(search || levelFilter) && (
                <button type="button" onClick={() => setShowAdd(true)} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontWeight: 900, fontSize: "inherit" }}>Add the first cinema video</button>
              )}
            </span>
          </div>
        )}
      ...
      {(showAdd || editing) && (
        <CinemaModal initial={editing ?? undefined} onSave={handleSave} onClose={() => { setShowAdd(false); setEditing(null); }} />
      )}
      {deleting && (
